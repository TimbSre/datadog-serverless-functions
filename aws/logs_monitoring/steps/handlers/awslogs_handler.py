...
class AwsLogsHandler:
    ...
    def handle_step_function_source(self, metadata, aws_attributes):
        if not self.cache_layer.get_step_functions_tags_cache().should_fetch_tags():
            # If not tags enrichment done locally
            # We let Datadog enrich it with Resource Collection data
            return
